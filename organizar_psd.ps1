param(
    ...
    [string]$DestinationPath = "C:\Users\guillermo matute\Desktop\PSD_Organizados"
)

# Definir carpetas a excluir (sin caracteres especiales)
$ExcludePaths = @(
    "AppData",
    ".gemini",
    "node_modules",
...
Write-Host "--- ORGANIZADOR DE ARCHIVOS PSD ---" -ForegroundColor Cyan
...
Write-Host "Buscando archivos PSD..." -ForegroundColor Cyan
# Obtener todos los archivos PSD de forma recursiva
$allFiles = Get-ChildItem -Path $SearchPath -Filter *.psd -Recurse -File -ErrorAction SilentlyContinue
...
Write-Host "Se encontraron $totalCount archivos PSD." -ForegroundColor Green

if ($totalCount -eq 0) {
    Write-Host "No se encontraron archivos PSD para organizar." -ForegroundColor Yellow
    exit
}
...
foreach ($file in $sortedFiles) {
    # Formatear el indice (ej: 001, 002)
    ...
    # Registrar la accion sin usar el caracter tubería literal en interpolación compleja
    $origPath = $file.FullName
    $logLine = "{0} - {1} - Original: {2} -> Nuevo: {3}" -f $indexString, $fileDate, $origPath, $newName
    $logEntries += $logLine
...
if (-not $Simulate) {
    ...
    $header = @(
        "==================================================",
        "REGISTRO DE ORGANIZACION DE ARCHIVOS PSD",
